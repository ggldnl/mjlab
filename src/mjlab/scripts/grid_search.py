  ...
  def step(self, actions: torch.Tensor):
    ...
    if ep_rewards_done:
      ...
      if self._KEY_REWARD_STD in self._accumulators and len(self._done_rewards) > 1:
        ...
        variance = sum((r - overall_mean) ** 2 for r in self._done_rewards) / len(
          self._done_rewards
        )
...
class SearchOnPolicyRunner(MjlabOnPolicyRunner):
  ...
  def learn_with_search(
    ...
    while remaining > 0:
      ...
      metrics = {
        k: v
        for k, v in ((k, acc.pop()) for k, acc in self._accumulators.items())
        if v is not None
      }
      if not metrics:
        continue
...
def _save_best(
  path: Path, trial_n: int, score: float, weights: dict[str, float]
) -> None:
...
def _make_objective(
  ...
  def objective(trial: optuna.Trial) -> float:
    ...
    runner = SearchOnPolicyRunner(
      env,
      agent_dict,
      str(trial_dir),
      device,
      metric_keys=[m.key for m in search_cfg.metrics],
    )
    ...
    try:
      ...
      print(
        f"Trial {trial.number:04d} pruned at iter {runner.current_learning_iteration}."
      )
      env.close()
      raise
