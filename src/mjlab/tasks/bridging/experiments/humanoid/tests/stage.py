...
Run:
    ...
    uv run python -m ...transitions.walk2kick
...
one instant, which is what the bridge trains on. It comes from the entering skill itself.
`profile` rolls that skill out once from its own reset, and any frame of the rollout is a
state it is known to work from. Picking one is what a real controller would have to decide;
here it is a slider.

From the rollout, and pointedly not from whatever reference the skill was tracking. A
retargeted clip is a description of a motion, not a state a robot is ever in: the jump's,
at frame 90, stands nearly four centimetres into the floor.

The `preview` slider stands a second ghost in whichever frame it is on, cool where the
target's is warm, and where the rollout was recorded rather than where the crossing is
going. The target is that same frame moved rigidly, so the preview shows what the bridge
will be sent to.
...
class Actor(NamedTuple):
  ...
  Called as `enter(env, frame, pos, heading)`: the profile frame it takes over at, the world
  position the robot will be at, and the direction it should head. A skill with no reference
  and no episode state of its own does not need one.

  Placement only. What state the robot has to arrive in is `aim`'s to say, from the profiled
  rollout: a state this skill was measured holding. A reference is not one, and a retargeted
  clip is not one by several centimetres of floor.
...
class Couple:
  ...
  steps: int = 30
  frame: int = 50
  """This couple's window: how long the bridge gets, and which frame of the entering skill's
  profiled rollout to aim at. Slider defaults, overridable on the command line.

  Hardcoded per couple because choosing them is a real decision, not one this harness makes.
  Which state of the entering skill is worth arriving in depends on how much momentum there
  is to shed and what the skill is doing at that frame. Getting it wrong does not look like
  a bad choice, it looks like a broken bridge. The push shows it:

      frame 50, 30 steps    crate already moving at 1.1 m/s, so shed 2 m/s in 0.6 s
                            (3.3 m/s^2, past what a body does)      score 0.30
      frame 0, 35 steps     the stance the push actually opens from  score 0.81

  Same bridge, both times. A learned component should make this choice eventually, from the
  state the robot is in, the frames on offer and the time each would need. Until then it is
  two numbers per script."""
...
"""The preview ghost. Cool and fainter than the target, since the two share the screen and
are not the same kind of thing: one is where the bridge is being sent, the other is a frame
of a rollout being looked through."""
...
class Aimed(BridgeCommand):
  ...
  def open_window(self, env_ids: torch.Tensor, deadline: torch.Tensor) -> None:
    """Start the clock on a target that was placed from outside.

    What `place` does, minus the two things this arena cannot have. It draws no pair, since
    `aim` has already written the target, and it teleports nobody, since the robot is
    wherever the leaving skill left it and that is the point of the test.
    """
    self.deadline[env_ids] = deadline
    self._opened[env_ids] = self._env.episode_length_buf[env_ids]
    self.start_distance[env_ids] = (
      self.robot.data.root_link_pos_w[env_ids] - self.target[env_ids, 0:3]
    ).norm(dim=-1)
  ...
  def _debug_vis_impl(self, visualizer: DebugVisualizer) -> None:
    ...
    velocity and a still body says nothing about that. It does show the two things the
    sliders move, where `aim` decided a body carrying this momentum could be after `steps`
    and which of the entering skill's frames it has to hold there, and it stays put after
    the hand-over so the gap to the robot is the arrival error left standing.
    ...
    The preview is the same rollout `frame` indexes into, in its own colour, left where it
    was recorded rather than moved to where the crossing is going. Scrubbing it walks the
    entering skill through its own run, so which frame to aim at can be decided by looking
    instead of by pressing the button and finding out.
    """
    if self.aimed:
...
def arena(couple: Couple) -> ManagerBasedRlEnvCfg:
  ...
  # The dataset comes along exactly as the bridge trained with it, and is read here for its
  # frame rate and joint count only. No window is ever drawn in this arena; Aimed takes its
  # target from outside
  cfg.commands["bridge"] = AimedCfg(**aimed)
  cfg.observations = {BRIDGE_GROUP: cfg.observations["actor"]}

  # Nothing is trained and nothing should reset: a fall is the result of the test, not an
  # error to recover from, and a reset mid-run would move the robot out from under the
  # phase machine
  cfg.rewards = {}
  cfg.terminations = {}
...
def aim(
  ...
  """Point the bridge at a frame of the entering skill's rollout, moved to meet the robot.
  ...
  This replaced reading the target off the entering skill's reference, through a return
  value from `Actor.enter`. For a clip tracker that hands the bridge a retargeted human
  motion, which no robot is ever in. At frame 90 of the jump, clip against policy:
...
def profile(
  ...
  The skill's initiation set, sampled: the states it puts itself in when nothing has gone
  wrong, and therefore the states it is known to work from. One of them becomes the bridge's
  target.
  """
  env.reset()
...
class Run:
  ...
  def preview(self) -> int:
    ...
    part of the transition. It exists because `frame` picks a state out of that rollout
  ...
  def triggered(self) -> bool:
    ...
    switch fires when walking has brought the object into that place. That is what a
    controller would be deciding, reduced to the one rule that makes the scenario run. The
    button and --auto are for skills with no such rule, and an override for those with one.
    """
    ...
    if ready is None:
      return False
    here = state(self.robot)
    # Where the robot will be when control changes, which is not where aim puts the target:
    # half the ballistic lead, which is what a target with a standing object's own velocity
    # gets, plus whatever the bridge is known to overshoot by
    at = here[:, 0:3].clone()
    lead = (self.steps / self.command.fps) / 2.0
  ...
  def verdict(self, here: torch.Tensor) -> str:
    ...
    The band is what training drew: deadlines uniform over `deadline_range` and nothing
    else, because every window it saw was already feasible when the pair was built. Outside
    that band the bridge is being asked something it was never shown, so a bad score there
    is evidence about the slider.
    ...
    seconds = self.steps / self.command.fps
    accel = change / seconds

    said = f", sheds {change:.1f} m/s in {seconds:.2f} s ({accel:.1f} m/s^2)"
    low, high = cfg.deadline_range
    if not low <= self.steps <= high:
      return f"{said}: {self.steps} steps is outside the {low}-{high} it trained on"
    if accel > MAX_ACCEL:
  ...
  def hand_over(self):
    ...
    errors = channel_errors(now.unsqueeze(0), want.unsqueeze(0), self.robot.num_joints)
...
def panel(server, run: Run) -> None:
  ...
  with server.gui.add_folder(f"Hand over to {run.couple.entering.name}"):
    ...
    for name, lo, hi in (
      ("steps", *run.command.cfg.deadline_range),
