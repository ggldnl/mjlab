"""Plan once with diffusion and track the path with an injected feedback executor."""
...
class DiffusionRuntime(Bridge):
  """Generate a kinematic path and pass a moving reference to a feedback policy."""
  ...
  def set_executor(self, executor: PathExecutor) -> None:
  ...
  def load(self, device: torch.device | str) -> DiffusionBridge:
    if self._bridge is None:
      path = find_checkpoint(
        ...
        hint=" Train one with bridges.diffusion.train.",
      )
      print(f"bridge   {path}")
  ...
  def _reference(self) -> torch.Tensor:
    ...
    rows = (self._index[:, None] + 1 + offsets).clamp_max(self.path.states.shape[1] - 1)
  ...
  def step(
    ...
    if self.executor is None:
      raise RuntimeError(
        "DiffusionRuntime needs a feedback path executor. Install the replacement "
        "tracker with set_executor(); v4 never replays generated actions open loop."
      )
    current = history[:, -1]
    ...
    return BridgeOutput(
      ...
      captured=torch.zeros_like(played, dtype=torch.bool),
