...
MuJoCo model. This script does that translation; the fetching is the tracking task's ASAP
downloader, which this calls.
...
2. Convert the turning and sideways jumps too, which widen the goal space.
    ...
    uv run --with joblib python -m \
      mjlab.tasks.bridging.experiments.humanoid.skills.jump_continuous.dataset --clips all
"""
...
DEFAULT_OUTPUT_DIR = Path("data") / "asap" / "motions"

# Which clips exist, where they come from and how they are fetched all live in the ASAP
# downloader. This file only says which set to convert: "forward" is the five forward jumps
# that make up the goal-conditioning axis, "all" adds the turning and sideways ones
CLIP_SETS = asap.CLIP_SETS
...
def convert(
  ...
  """Fetch the named ASAP clips and convert them into mjlab motion npz files.

  One MuJoCo model is built and every clip is replayed through it, so converting a set costs
  the setup once. Called by main below with a whole clip set, and by the single clip jump
  with its one clip.

  Args:
    names: Clip names, as the ASAP downloader spells them.
  ...
  converted = []
  print(f"Converting {len(names)} clips to {output_dir}:")
  for name in names:
    source = asap.fetch(name, cache_dir, ref)
    converted.append(
      convert_clip(
        sim=sim,
        scene=scene,
  ...
  # Merged into whatever is already there rather than replacing it. Converting one clip is
  # a normal thing to do here, and a manifest listing only that clip would leave every other
  # npz in the directory unordered, which is what discover_motion_files reads it for
  manifest_path = output_dir / "manifest.json"
  existing = json.loads(manifest_path.read_text()) if manifest_path.exists() else []
  by_file = {entry["file"]: entry for entry in existing}
  by_file.update({entry["file"]: entry for entry in converted})
  manifest = sorted(by_file.values(), key=lambda entry: entry["distance"])
...
def main(
  ...
  """Convert a named set of ASAP jump clips.
  ...
  Args:
    ...
    device: Torch device for the replay.
  """
  convert(CLIP_SETS[clips], output_dir, cache_dir, output_fps, align, ref, device)
