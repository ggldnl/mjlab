...
3. Watch.
    ...
    uv run play Mjlab-G1-Jump-Continuous --checkpoint-file <path>
"""
...
##
# The two observations, and how they differ.
##
...
def g1_jump_continuous_env_cfg(
  ...
  One environment, two observations, because one task trains two policies in one run. The
  tracking phase learns against "teacher", which carries the reference; the distillation
  ...
  # Three groups, because this task trains two policies in one run. See
  # RslRlTeacherStudentRunnerCfg for the phases and skills/jump for why the jump needs them.
  #
  # "actor" is the deployed policy's, and it is called that because it is what every
  # inference path asks for: `play`, and the composition arena, load an actor and get the
  # distilled student without knowing there were two phases. "teacher" is what the tracking
  # phase learns against, and is the observation this task had before it grew a second phase
  teacher_group = ObservationGroupCfg(
