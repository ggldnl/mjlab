"""What the kick adds to the pass: a place on the foot to strike with, and a strike
that has to be fast.
...
So the kick measures the approach from the inside of the foot, charges for contact made
with that surface moving slowly, and pays a rung of reward for the ball reaching the speed
it was asked for. The ball itself
stays inside the reach of a standing robot, which is the one thing an earlier version of
this task got wrong: a ball at the edge of the reach envelope makes approach_inside_foot a gradient
...
"""The body the striking surface belongs to. The pass measures from a `right_foot` site
sitting at (0.04, 0, -0.037) in this frame, which is the middle of the sole."""

INSIDE_FOOT_OFFSET = (0.075, 0.03, -0.02)
"""Where the striking surface is, in the striking body's frame, in metres.

The arch, on the medial side of the right foot, which is the surface a person passes a ball
with. Read off the collision geometry rather than chosen: right_foot7_collision is the
innermost capsule, running x = 0.05 to 0.1 at y = 0.026 with a 0.01 radius, so the inside
surface of the foot is at y = 0.036 and this sits 0.006 within it, halfway along.

That it faces sideways is the point, and is what makes this a different motion from a toe
poke. To bring this surface to a ball in front of the robot the hip has to rotate the leg
outward first, which is the technique being asked for, and the reward gets it for free
because the gap it measures cannot close any other way.
...
STRIKE_SPEED = 1.5
"""Speed of the striking surface, in m/s, above which a contact counts as a strike.
...
the low end of the command range takes a foot doing two to three, so this sits between the
...
def _inside_foot_offset_w(env: ManagerBasedRlEnv) -> tuple[Entity, int, torch.Tensor]:
  """The striking body, its index, and the contact offset rotated into world axes."""
  robot: Entity = env.scene[ROBOT]
  index = robot.body_names.index(STRIKE_BODY)
  offset = torch.tensor(INSIDE_FOOT_OFFSET, device=env.device).expand(env.num_envs, 3)
  return robot, index, quat_apply(robot.data.body_link_quat_w[:, index], offset)


def inside_foot_pos_w(env: ManagerBasedRlEnv) -> torch.Tensor:
  """World position of the striking surface, shaped (num_envs, 3).
  ...
  not to: one skill's idea of where it strikes from does not belong in an asset eleven other
  environments load. Add one temporarily if you want to see it in the viewer.
  """
  robot, index, offset_w = _inside_foot_offset_w(env)
  return robot.data.body_link_pos_w[:, index] + offset_w


def inside_foot_vel_w(env: ManagerBasedRlEnv) -> torch.Tensor:
  """World velocity of the striking surface, shaped (num_envs, 3).

  The body's own velocity plus the rotational contribution, which is not a refinement here
  but most of the quantity. A kick is mostly ankle, knee and hip rotation, so a point held
  out from the body frame gets much of its speed from the cross product rather than from the
...
def inside_foot_speed(env: ManagerBasedRlEnv) -> torch.Tensor:
  """How fast the striking surface is moving, shaped (num_envs,)."""
  return torch.norm(inside_foot_vel_w(env), dim=-1)


def inside_foot_pos_b(env: ManagerBasedRlEnv) -> torch.Tensor:
  """The striking surface in the robot's heading frame, shaped (num_envs, 3)."""
  robot: Entity = env.scene[ROBOT]
  return quat_apply_inverse(
    heading_quat(env), inside_foot_pos_w(env) - robot.data.root_link_pos_w
  )


def inside_foot_vel_b(env: ManagerBasedRlEnv) -> torch.Tensor:
  """The striking surface's velocity in the heading frame, shaped (num_envs, 3).
  ...
  """
  return quat_apply_inverse(heading_quat(env), inside_foot_vel_w(env))


def inside_foot_height(env: ManagerBasedRlEnv) -> torch.Tensor:
  """Height of the striking surface above the floor, shaped (num_envs, 1).

  No longer a gate on anything, kept as an observation. Foot clearance is what decides
  whether a swing meets the ball or the floor first, and the z in inside_foot_pos_b is
  measured from a root that moves, so it does not answer the same question.
  """
  return inside_foot_pos_w(env)[:, 2:3]


def inside_foot_gap(env: ManagerBasedRlEnv) -> torch.Tensor:
  """Distance from the inside of the foot to the ball's surface. (num_envs,).

  The pass's ball_surface_gap with the arch in place of the mid-sole site. Same floor at zero
  and same reason for subtracting the radius: a kernel on the centre distance peaks at a
  value no foot can reach.
  """
  gap = torch.norm(ball_pos_w(env) - inside_foot_pos_w(env), dim=-1) - BALL_RADIUS
...
def approach_inside_foot(env: ManagerBasedRlEnv, std: float) -> torch.Tensor:
  """The pass's approach term measured from the arch. Shaped (num_envs,).
  ...
  """
  score = torch.exp(-torch.square(inside_foot_gap(env)) / std**2)
...
def shove_cost(env: ManagerBasedRlEnv) -> torch.Tensor:
  """Touching the ball with a slow foot. Shaped (num_envs,), to be weighted down.
  ...
  Speed rather than height, which is what an earlier version of this gated on. A kick of a
  ball resting on the floor meets it below the ball's centre with the foot low, so a height
  gate charges the motion the task is asking for while a heel planted, toes up shovel passes
  it untouched. Speed makes no such mistake and needs no tuning against the ball's radius.
  ...
  above.
  """
  return (touching_ball(env) & (inside_foot_speed(env) < STRIKE_SPEED)).float()
