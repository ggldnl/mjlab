...
# The velocity task's terms come in first and the kick's are layered on top, so a config
...
# The foot that kicks, and the one holding the robot up while it happens. The ball spawns
# in front of the kicking foot only, so these are not interchangeable: moving the skill to
...
# this the kick reward stays at zero, so leaning on the ball earns nothing
LAUNCH_SPEED = 0.5

# Ball speed counting as disturbed during the stance window. Well under LAUNCH_SPEED: the
# point is to catch a robot creeping into the ball early, not to wait for a real kick
DISTURB_SPEED = 0.15
...
def heading_quat(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  kick, and a vector in the full base frame swings with it, so the ball would appear to move
...
class KickPhase:
  ...
  def __init__(self, env: ManagerBasedRlEnv) -> None:
    ...
    # the kick is scored on. Fastest rather than first means an early nudge is not locked in
    # as the answer: a real kick later in the episode replaces it
  ...
  def refresh(self, env: ManagerBasedRlEnv) -> None:
    ...
    # Gated on the kicking foot having touched the ball, and updated after that flag, so a
    ...
    # the kicking foot would then find max_speed already high and be paid in full for a kick
    # it did not make
...
def stance_progress(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Zero at reset, one when the kick window opens and after. This makes the gate legible to
  the policy. Without it the kick terms switch on at a moment the observation gives no way
...
class KickCommand(CommandTerm):
  ...
  heading the robot could satisfy a badly aimed kick by turning to face wherever the ball
  went.
  ...
  even if the robot turns while kicking.
  """
...
def ball_contact(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  than privileged. It tells the policy which side of the kick it is on, which separates "go
...
def approach_ball(env: ManagerBasedRlEnv, std: float) -> torch.Tensor:
  ...
  parked against the ball, the cheapest way to hold a contact and the opposite of a kick.
  """
...
def ball_touched(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  that can score the kick terms, and a one-shot bonus for it is a rounding error against a
...
def kick_quality(
  ...
  ball's speed bleeds off as it rolls: a live kernel peaks some time after the kick, when
...
def stay_put(env: ManagerBasedRlEnv, std: float) -> torch.Tensor:
  ...
  through a kick without being charged for the transient.
  """
...
def early_disturbance(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  The gate on the kick rewards makes an early kick worthless. This makes it costly, which is
...
def ball_out_of_range(env: ManagerBasedRlEnv, distance: float = 8.0) -> torch.Tensor:
  ...
  A time out, not a failure. The kick is already scored and latched by the time this fires,
  and terminating as a failure would bootstrap zero onto the end of the best rollouts the
  policy ever produces, teaching it that kicking the ball a long way is bad.
  """
...
def ramp(final_weight: float, stage_steps: int) -> list[dict]:
  ...
  half step is not decoration: turning a large kick weight on in one go hands the policy a
