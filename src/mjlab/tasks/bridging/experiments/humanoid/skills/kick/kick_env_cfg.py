...
BALL_FORWARD_RANGE = (0.28, 0.36)
"""Forward offset of the ball's centre from the striking foot's site, in metres.

Exactly the pass's 0.24 to 0.32 shifted out by the three and a half centimetres between
the site it measures from and the arch this task measures from, so the gap the policy has
to close is the same one the pass already solves. That is the point. The reach problem is not what
...
left is a swing. What actually happens is that `approach_inside_foot` is a dense position kernel
...
W_SHOVE = -1.0
"""Cost per step of holding the ball in contact with a slow foot.
...
termination and still collect `alive`, `stay_put` and a good part of `approach_inside_foot`, which
...
def g1_kick_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """The pass environment, moved onto the inside of the foot.

  Five changes and nothing else:

      the ball is placed further out by the arch's own offset, so the approach is unchanged
      `approach_ball` is measured from the arch instead of the mid-sole site
      `shove_cost` charges for a contact made with a foot that is not moving
  ...
  # The ball, further out by the arch's offset. Mutating the event's params rather than
  ...
  # The approach, measured from the arch. Same kernel width: the gap it measures is a gap to
  # the ball's surface either way, so the scale of the problem has not changed
  cfg.rewards["approach_ball"] = RewardTermCfg(
    func=mdp.approach_inside_foot, weight=W_APPROACH, params={"std": 0.15}
  )
  ...
  # window. Its velocity is there because that is what shove_cost gates on, and a gate the policy
  ...
  for group in ("actor", "critic"):
    ...
    terms["inside_foot_pos"] = ObservationTermCfg(func=mdp.inside_foot_pos_b)
    terms["inside_foot_vel"] = ObservationTermCfg(func=mdp.inside_foot_vel_b)
    terms["inside_foot_height"] = ObservationTermCfg(func=mdp.inside_foot_height)
