"""The push: what the robot sees, what it is paid for, and where the crate starts.

The goal is the twist. Walking already has one, the velocity task's commanded linear and
...
and the cheapest solution is to walk into it chest first.

Three terms carry it:

    reach_box       a distance kernel reaching one exactly when a hand is against the crate
    hands_on_box    pays while the contact holds
    push_tracking   pays for the crate moving the way it was asked
"""
...
# The velocity task's terms come in first and the push's are layered on top, so a config
...
def box_quat_w(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Full orientation, not the yaw. A cube shoved by a walking humanoid tips as often as it
  slides, and the surface distance below is only right if the box's own axes are where it
  thinks they are.
  """
  box: Entity = env.scene[BOX]
...
def hand_pos_w(env: ManagerBasedRlEnv, hands_cfg: SceneEntityCfg) -> torch.Tensor:
  ...
  The geom center, not the wrist body origin. This G1 has domes where rubber hands would be,
  seated 0.04 m out along the wrist axis, which is most of a hand radius, and the reach
  kernel below measures the last few centimetres before contact.
  """
...
def heading_quat(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Yaw only, never the full root orientation. A robot leaning into a crate pitches hard, and
  a vector in the full base frame swings with it, so the crate would appear to move when
  only the robot leaned.
  """
  robot: Entity = env.scene[ROBOT]
...
def box_surface_gap(
  ...
  Exterior distance, computed in the box's own frame:
      ...
      norm(clamp(abs(offset) - half_extents, min=0))

  Zero anywhere inside or on the surface.

  Centre to centre would not do. It bottoms out at half a metre for a crate this size, so a
  kernel on it peaks at a value nothing can reach and spends its useful range on offsets
  that all mean the same thing. Subtracting the extents puts the peak where contact happens.
  """
...
def hand_box_gap(env: ManagerBasedRlEnv, hands_cfg: SceneEntityCfg) -> torch.Tensor:
  ...
  Measured to the sphere's centre, so it floors at the hand radius rather than zero when the
  hand rests on the crate. That offset is 0.03 m against a kernel half a metre wide, which
  is nothing, and leaving it in keeps this a plain distance readable off a log without
...
def commanded_vel_xy(
  ...
  push_env_cfg.py are strictly forward and no env is asked to stand still.
  """
...
def hand_box_offset_b(
  ...
  Redundant in principle: the policy is told where the crate is and knows its own joint
  angles. Given anyway because it is the exact quantity the reach kernel scores, and asking
  a network to rediscover its own forward kinematics before earning the first rung is a slow
  way to begin.
  """
...
def hands_contact(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  A contact switch on each palm is something a real robot can have, so this is ordinary
  rather than privileged. It separates "reach for the crate" from "lean on it and walk".
  """
...
def hands_contact_force(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Privileged. How hard the robot leans is most of what decides whether the crate moves, and
  it stands in for the crate's mass, which the reset randomizes and the actor is never told.
  Compressed the way the velocity task compresses foot forces, because the raw number spans
  two orders of magnitude between a brush and a shove.
  """
  sensor: ContactSensor = env.scene[HANDS_BOX_SENSOR]
...
def body_contact(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Privileged, because a G1 has no skin. The actor is penalized without being told directly,
  which is fair: it knows where the crate is and where its own limbs are, so a torso against
  the crate is predictable rather than something it has to be informed of. The critic gets it
  because it is a step change in the return, and guessing it from the rest of the state is
  wasted capacity.
  """
...
def hands_on_box_reward(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  The middle rung. A fraction rather than a flag, so the second hand is worth as much as the
  first. Contact is the discrete step between a policy that can reach for a crate and one
  that can move it, and the distance kernel above cannot express it: the last few
  centimetres of gap and the first newton of contact look nearly identical to it.
  """
...
def body_contact_penalty(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Paired with a negative weight, and the term the whole task turns on. A G1 walking into a
  metre of cube meets it with chest, hips and shins long before its palms, and shoving with
  the torso is easier to discover and easier to balance through than shoving with two 3 cm
  spheres at the end of two arms.

  A flag rather than a count of offending parts. Counting makes a policy that has already
  fallen against the crate pay a penalty growing with how much of it touches, which is a
  large negative spike arriving when the episode is already lost. The flag is a flat cost
  for a state the policy should never be in.
  """
...
def push_tracking(
  ...
  it slide off. Clamped below at zero, so a crate drifting backwards is worth the same as a
  crate standing still rather than a penalty the policy can run up.

  The contact gate stops the ungated version paying for a crate still coasting from a body
  check the policy was already penalized for. It costs a little smoothness, since a
...
def hands_contact_rate(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  The first thing to read. Everything below it is meaningless until it is high, because a
  policy that never reaches the crate is being scored on a crate nothing is touching. One
  half is a one handed push, a partial success. Near zero with the velocity tracking reward
  climbing is a policy that learned to walk around the crate.
  """
  return hands_on_box(env).float().mean(dim=-1)
...
def body_contact_rate(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  The second thing to read, and the one saying whether the hands-only constraint took. It
  should fall toward zero. High while the displacement metric also climbs means the policy
  is paying the penalty and body checking the crate anyway, and the fix is a larger
  body_contact weight, not a smaller push one.
  """
  return body_on_box(env).float()
...
def box_displacement(env: ManagerBasedRlEnv) -> torch.Tensor:
  ...
  Logged with reduce="max", so it reads as the furthest the crate got this episode rather
  than an average over a trajectory that started at zero. The headline number: metres of
  crate moved.
  """
...
def reset_box_in_front(
  ...
  Relative to the robot, never at an absolute point. The robot resets with a random yaw and
  a scattered position, so a fixed world pose would put the crate behind it as often as in
  front, and a crate the robot never meets is a guaranteed miss rather than a task.

  The forward offset is centre to centre, so the near face sits one half-extent closer than
  the number says:

      spawn 1.4 m   ->   robot 0.9 m from the face, hands about 0.5 m from it
  """
  if env_ids is None:
...
def ramp(final_weight: float, stage_steps: int) -> list[dict]:
  ...
  The half step is not decoration: turning a large weight on in one go hands the policy a
  reward it has no idea how to earn, and the cheapest way to look for it is to throw itself
  at the crate, which undoes the walking it just learned.
  """
  return [
    {"step": 0, "weight": 0.0},
